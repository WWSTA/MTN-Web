...
// GitHub Pages adaptation: resolve WASM paths relative to the page
const ORT_WASM_PATH = (() => {
  try {
    const scriptUrl = import.meta.url;
    const baseUrl = scriptUrl.substring(0, scriptUrl.lastIndexOf('/'));
    return baseUrl + '/vendor/ort/';
  } catch (_) {
    return './js/vendor/ort/';
  }
})();

// GitHub Pages adaptation: resolve tokenizer sandbox path
const TOKENIZER_SANDBOX_URL = (() => {
  try {
    const scriptUrl = import.meta.url;
    const baseUrl = scriptUrl.substring(0, scriptUrl.lastIndexOf('/'));
    const parentUrl = baseUrl.substring(0, baseUrl.lastIndexOf('/'));
    return parentUrl + '/tokenizer_sandbox.html';
  } catch (_) {
    return './tokenizer_sandbox.html';
  }
})();
...
function assetRootPathToUrl(pathValue) {
  ...
  // GitHub Pages adaptation: relative paths (e.g. "./models" or "models")
  if (normalized.startsWith("./") || normalized.startsWith("../") || !normalized.includes("://")) {
    const relative = normalized.endsWith("/") ? normalized : `${normalized}/`;
    // Use a relative URL that works with same-origin fetch
    try {
...
function ensureTokenizerSandboxFrame() {
  ...
  tokenizerSandboxReadyPromise = new Promise((resolve, reject) => {
    ...
    iframe.src = TOKENIZER_SANDBOX_URL;
